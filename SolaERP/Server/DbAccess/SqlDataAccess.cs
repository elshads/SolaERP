namespace SolaERP.Server.DbAccess;
...
public class SqlDataAccess
{
    ...
    public async Task<SqlResult?> ExecuteSql(string sql, DynamicParameters p)
    {
        SqlResult? result = new();
        try
        {
            using IDbConnection cn = new SqlConnection(ConnectionString);
            result.QueryResult = await cn.ExecuteAsync(sql, p, commandType: CommandType.StoredProcedure);
        }
        catch (Exception e)
        {
